...
import { StatusbarEvents, HeaderEvents, TitlebarEvents, PanelEvents } from "@enjoys/monaco-vanced/core/events";
...
export function wireTitleBar(dom: DOMRefs, apis: WireframeAPIs, eventBus: EventBus, on: OnHandler) {
  ...
  for (const label of MENUS) {
    const item = el("div", {
      class: "vsc-menu-item",
      style: `padding:2px 8px;cursor:pointer;font-size:12px;color:${C.fg};user-select:none;-webkit-app-region:no-drag;`,
    }, label);
    item.addEventListener("click", () => {
      ...
      // Terminal → Toggle Bottom Panel and switch to terminal tab
      if (label === "Terminal") {
        eventBus.emit(PanelEvents.BottomToggle, {});
      }
    });
